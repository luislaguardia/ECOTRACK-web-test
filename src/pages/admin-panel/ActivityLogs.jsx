...
const ActivityLogs = () => {
  ...
  const { filteredLogs, actionTypes, stats } = useMemo(() => {
    const filtered = logs.filter(log => {
        const details = log.details?.toLowerCase() || "";
        const action = log.action?.toLowerCase() || "";
        const matchesSearch = searchTerm === "" || details.includes(searchTerm.toLowerCase()) || action.includes(searchTerm.toLowerCase());
    ...
    return { filteredLogs: filtered, actionTypes: types, stats: { logsLast24h } };
  }, [logs, searchTerm, selectedAction, dateRange]);
  ...
  if (isLoading) {
      return (
        <div className="p-6 bg-[#F5F5F5] min-h-screen">
          <div className="mb-6">
            <h1 className="text-3xl font-bold text-slate-800">My Activity Log</h1>
            <p className="text-slate-600 mt-1">Track your personal activities and actions in the system.</p>
          </div>
  ...
  return (
    <div className="p-6 bg-[#F5F5F5] min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
          <StatCard title="Total Activities" value={logs.length} icon={<FileText className="w-6 h-6 text-blue-500" />} color="bg-blue-100" />
          <StatCard title="Unique Actions" value={actionTypes.length} icon={<Activity className="w-6 h-6 text-green-500"/>} color="bg-green-100"/>
          <StatCard title="Activities in Last 24 Hours" value={stats.logsLast24h} icon={<Clock className="w-6 h-6 text-yellow-500"/>} color="bg-yellow-100"/>
      </div>
            ...
            {paginatedLogs.length > 0 ? (
              ...
              <tr>
                <td colSpan="5" className="px-6 py-12 text-center text-slate-500">
                  ...
                  <p>No activity logs were found matching the current filters.</p>
                </td>
              </tr>
            )}
          </tbody>
